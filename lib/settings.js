...
function seedFromEnv() {
    ...
    if (iplocateKey) s.apiKeys.iplocate = splitKeys(iplocateKey);
    const ip2locationKey = process.env.IP2LOCATION_API_KEY;
    if (ip2locationKey) s.apiKeys.ip2location = splitKeys(ip2locationKey);
    const ipdataKeys = process.env.IPDATA_API_KEYS || process.env.IPDATA_API_KEY;
    if (ipdataKeys) s.apiKeys.ipdata = splitKeys(ipdataKeys);
...
function normalizeApiKeys(settings) {
    for (const name of ['abuseipdb', 'iplocate', 'ip2location', 'ipdata']) {
        const value = settings.apiKeys?.[name];
        if (Array.isArray(value)) continue;
        settings.apiKeys[name] = splitKeys(value);
    }
}
...
function migrateSeo(settings, saved) {
    ...
    if (!savedSeo.localized && (savedSeo.title || savedSeo.description || savedSeo.keywords)) {
        ...
        if (containsCjk(legacySeo)) {
            settings.seo.localized.zh = legacySeo;
        } else {
            settings.seo.localized.en = legacySeo;
        }
    }
    repairEnglishSeo(settings);
...
function repairEnglishSeo(settings) {
    const en = settings.seo.localized?.en;
    if (!en || !containsCjk(en)) return;

    const defaults = DEFAULT_SETTINGS.seo.localized;
    const zh = settings.seo.localized.zh || {};
    if (!zh.title || isSameSeo(zh, defaults.zh)) {
        settings.seo.localized.zh = { ...en };
    }
    settings.seo.localized.en = { ...defaults.en };
}

function containsCjk(seo) {
    const text = [seo.title, seo.description, seo.keywords].filter(Boolean).join(' ');
    return /[\u3040-\u30ff\u3400-\u9fff\uf900-\ufaff]/.test(text);
}

function isSameSeo(a, b) {
